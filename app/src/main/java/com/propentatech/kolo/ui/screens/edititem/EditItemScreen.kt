package com.propentatech.kolo.ui.screens.edititem
...
fun EditItemScreen(
    ...
    // Initialiser les valeurs quand l'élément est chargé
    LaunchedEffect(item) {
        item?.let {
            title = it.title
            amount = if (it.requiresSaving) it.amount.toString() else ""
            requiresSaving = it.requiresSaving
        }
    }

    // Si l'élément n'est pas encore chargé, afficher un écran vide
    if (item == null) {
        Scaffold(
            topBar = {
                TopAppBar(

...
fun SavingsHistoryScreen(
    ...
    var entryToDelete by remember { mutableStateOf<SavingEntryEntity?>(null) }

    // Modal de confirmation pour supprimer une épargne
    entryToDelete?.let { entry ->
        AlertDialog(
            onDismissRequest = { entryToDelete = null },
            title = { Text(strings.delete) },
            text = { Text("${strings.projectDeleteConfirm} cette épargne de ${KoloUtils.formatAmountWithCurrency(entry.amount, strings.currency)} ?") },
            confirmButton = {
                TextButton(
                    onClick = {
                        viewModel.deleteSavingEntry(entry)
                        entryToDelete = null
                    }
                ) {
